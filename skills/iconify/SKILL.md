...
```bash
python3 skills/iconify/scripts/iconify.py search "gauge" --limit 8 --prefixes lucide,tabler,ph,mdi,carbon
```

2. Passenden Treffer lokal speichern:

```bash
python3 skills/iconify/scripts/iconify.py download lucide:gauge --output assets/gauge.svg
```

3. Nur bei klarer Semantik direkt waehlen und speichern:

```bash
python3 skills/iconify/scripts/iconify.py pick "calendar check" --output assets/calendar-check.svg --limit 12
```
...
```python
from skills.iconify.scripts.iconify import fetch_svg_icon
